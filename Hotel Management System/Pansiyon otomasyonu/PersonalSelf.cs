...
{
    public partial class PersonalSelf : Form
    {
        public PersonalSelf()
        {
            InitializeComponent();
        ...
        private void PersonalSelf_Load(object sender, EventArgs e)
        {
            TxtUserName.Text = PersonLogın.username;
            TxtUserName.Enabled = false;
            SqlCommand cmd = new SqlCommand("select PName, PSurname, PMail, PPhone, PImage from tbl_Personal where pUsername=@p1", con.bgl());
        ...
        private void BtnUpdate_Click(object sender, EventArgs e)
        {
            if (TxtPass.Text != "")
            {
                SqlCommand cmd = new SqlCommand("Update tbl_Personal set pImage=@p1, pName=@p2, pSurname=@p3, pPass=@p4, pMail=@p5, pPhone=@p6 where pUsername=@p7", con.bgl());
