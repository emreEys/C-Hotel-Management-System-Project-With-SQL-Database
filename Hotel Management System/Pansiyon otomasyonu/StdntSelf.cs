...
{
    public partial class StdntSelf : Form
    {
        public StdntSelf()
        {
            InitializeComponent();
        ...
        private void StdntSelf_Load(object sender, EventArgs e)
        {
            TxtUserName.Text = StdntLogın.username;
            TxtUserName.Enabled = false;
            SqlCommand cmd = new SqlCommand("select sName, sSurname, sMail, sPhone, sImage from tbl_Stdnt where sUsername=@p1", con.bgl());
        ...
        private void BtnUpdate_Click(object sender, EventArgs e)
        {
            if (TxtPass.Text != "")
            {
                SqlCommand cmd = new SqlCommand("Update tbl_Stdnt set sImage=@p1, sName=@p2, sSurname=@p3, sPass=@p4, sMail=@p5, sPhone=@p6 where sUsername=@p7", con.bgl());
